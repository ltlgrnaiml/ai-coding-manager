...
from contracts.adr_schema import (
...
from contracts.plan_schema import PlanSchema
from contracts.spec_schema import SPECSchema
...
def get_knowledge_context(prompt: str, max_tokens: int = 4000) -> dict | None:
    ...
    """
    try:
        from backend.services.knowledge.context_builder import ContextBuilder
        from backend.services.knowledge.database import init_database
        from backend.services.knowledge.sanitizer import Sanitizer
        from backend.services.knowledge.search_service import SearchService
...
async def get_artifact(artifact_id: str) -> dict[str, Any]:
    ...
    """
    from backend.services.workflow_service import get_generated_content
...
async def generate_artifacts_endpoint(request: GenerationRequest) -> GenerationResponse:
    ...
    from contracts.devtools.workflow import GenerationStatus
...
async def set_llm_model(request: SetModelRequest) -> SetModelResponse:
    ...
    """
    from backend.services.llm_service import get_model_info, set_current_model
...
async def get_llm_usage() -> LLMUsageStats:
    ...
    """
    from backend.services.llm_service import get_llm_usage_stats
