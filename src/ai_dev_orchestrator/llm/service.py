"""LLM Service - xAI (Grok) integration for structured output generation.

This service provides:
- Structured output generation with Pydantic schema validation
- Retry logic with error feedback
- SQLite logging for all API calls
- Cost estimation and usage tracking
- Health checking and model management

Example:
    from ai_dev_orchestrator.llm import generate_structured, check_health
    ...
    if check_health().status == LLMStatus.AVAILABLE:
        response = generate_structured("Create a task for...", TaskOutput)
        if response.success:
            print(response.data)
"""
...
# Default workspace directory for logs
WORKSPACE_DIR = Path(os.getenv("AI_DEV_WORKSPACE", ".workspace"))
LLM_LOG_DB = WORKSPACE_DIR / "llm_logs.db"
...
class ModelInfo:
    ...
    Attributes:
        ...
        tpm: Tokens per minute rate limit (None if unlimited).
        rpm: Requests per minute rate limit.
        input_price: Price per million input tokens.
        output_price: Price per million output tokens.
        category: Model category (fast, premium, budget, code, vision).
        reasoning: Whether model has reasoning capabilities.
    """
    id: str
    name: str
    context_window: int
...
def _log_llm_call(
    ...
    if cost_estimate:
        logger.info(f"Logged LLM call #{row_id}: model={model}, success={success}, cost=${cost_estimate:.6f}")
    else:
        logger.info(f"Logged LLM call #{row_id}: model={model}, success={success}")
    return row_id
