...
6. **Lessons Learned / Solved Problems** (future)
   - Capture the self-reflection phase after problem resolution
   - **Problem**: What went wrong / what was unclear
   - **Diagnosis**: Decision process for how to find the solution
   - **Resolution**: Steps taken to solve the problem
   - **Pattern**: Distilled, reusable insight for future teams
   - Semantic search to surface relevant past solutions
   - Auto-suggest when current context matches a solved problem
   - Closes the feedback loop: Prompt → Trace → Response → Evaluation → **Lesson**
